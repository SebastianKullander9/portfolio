...
void main() {
    ...
    for(int i = 0; i < 3; i++) {
        ...
        float noiseSpeed = uSpeed * (0.2 + float(i) * 2.0);
        float noiseSeed = 100.0 + float(i) * 50.0;

        // Increase these multipliers for smaller color patches
        vec2 noiseCoord = vUv * vec2(20.0 * scale, 12.0 * scale);

        // Increase these frequencies too
        float noise = snoise(vec3(noiseCoord.x * 0.4 + uTime * noiseFlow, noiseCoord.y * 0.6, uTime * noiseSpeed + noiseSeed));

        colorNoise += noise * 0.5;
        scale *= 1.5;
    }
